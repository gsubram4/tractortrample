...
public class LaserFenceFactoryLine implements IntervalFactory {
	...
	public Interval createInterval(float xPos, float yPos, SlidingSpeed slidingSpeed) {
		...
		Entity[] output = new Entity[1];
			output[0] = LaserFence.laserFactory().setPosition(xPos + (0 * 150), yPos+160-(0*200)).setSlidingSpeed(slidingSpeed);
